defmodule Mix.Tasks.Phx.Gen.Auth do
  ...
  defp maybe_inject_app_layout_menu(%Context{} = context) do
    ...
    if api? do
      ...
      if file_path = get_layout_html_path(context) do
        ...
        #{schema.singular} menu items to be rendered.
  ...
  defp maybe_inject_fallback_controller(%Context{context_app: context_app} = context, paths, binding) do
    api? = Keyword.get(context.opts, :api, false)
    ...
    if api? do
      ...
      binding = [
        ...
        gettext?: Code.ensure_loaded?(Module.concat(context.web_module, "Gettext"))] ++ binding
